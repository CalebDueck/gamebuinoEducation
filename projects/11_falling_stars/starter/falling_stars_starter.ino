...
byte playerW = 8, playerH = 6;
int starX = 20, starY = 0;
byte starW = 4, starH = 4;
byte starSpeed = 1;
int score = 0;
byte lives = 3;
void setup() { gb.begin(); gb.pickRandomSeed(); gb.titleScreen(F("Falling")); }
void resetGame() {
  score = 0;
  lives = 3;
  playerX = 38;
...
void resetStar() {
  // CORE QUEST: choose a random x position and reset starY to 0.
...
void updatePlayer() {
  // CORE QUEST: move left and right, then add bounds.
}
void updateStar() {
  // CORE QUEST: make the star fall, catch it, and miss it.
}
void drawPlayer() { gb.display.fillRect(playerX, playerY, playerW, playerH); }
void drawStar() { gb.display.fillRect(starX, starY, starW, starH); }
void drawHud() {
  gb.display.print(F("S:")); gb.display.print(score);
  gb.display.print(F(" L:")); gb.display.println(lives);
}
void updatePlaying() {
  updatePlayer();
  updateStar();
